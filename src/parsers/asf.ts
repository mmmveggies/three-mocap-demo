// from https://raw.githubusercontent.com/encadyma/motion-out/master/src/parser/asf.js

import { toRadians } from './constants'
import * as THREE from 'three'

const TOKEN_KEYWORD_RGX = /^:(\w+)(?:\s+([\w\d\s.]+))?/
const TOKEN_COMMENT_RGX = /^#(.+)/
const TOKEN_BEGIN_RGX = /^begin$/
const TOKEN_END_RGX = /^end$/
const TOKEN_WHITESPACE_RGX = /^\s*$/
const TOKEN_TUPLE_RGX = /(\w+\s)?\(((?:\s?[\d.e-]+)+)\)/
...
export class ASFParser {
    ...
    while (index < this.tokens.length) {
      ...
      if (token.type == "KEYWORD") {
        ...
        switch (currentKey) {
          ...
          default:
            console.error(
              "warning! cannot process keyword " + JSON.stringify(token)
            );
            this.errors.push(
              "Cannot process keyword " +
        ...
        switch (token.words[0]) {
          case "mass":
            if (parseFloat(token.words[1]) == NaN) {
              console.error(
                "error! argument @ " + JSON.stringify(token) + " not a float!"
              );
              this.errors.push(
                "Argument to property `mass` @ " +
                  JSON.stringify(token) +
                  " on index " +
                  index +
                  " not a float."
        ...
        while (
          ...
          if (this.tokens[index].type == "PROPERTY") {
            ...
            switch (this.tokens[index].words[0]) {
              case "order":
                this.root.order = this.tokens[index].words
                  .slice(1)
                  .map((s) => s.toUpperCase());
                break;
              case "axis":
              ...
              default:
                console.error(
                  "[2] warning! in rootdata loop, cannot process property " +
                    JSON.stringify(this.tokens[index])
                );
                this.errors.push(
                  "Unexpected property " +
                    JSON.stringify(this.tokens[index]) +
            ...
            console.error(
              "[1] warning! in rootdata loop, cannot process keyword " +
                JSON.stringify(this.tokens[index])
            );
            this.errors.push(
              "Unexpected token " +
          ...
          if (this.tokens[index].type == "RELATION") {
            ...
            if (rel[0] == "root") {
              ...
              if (!traverse(this.tree)) {
                console.error(
                  "[2] warning! in hieradata loop, cannot find location for relation " +
                    JSON.stringify(this.tokens[index])
                );
                this.errors.push(
                  "Confused relation " +
            ...
            console.error(
              "[1] warning! in hieradata loop, cannot process keyword " +
                JSON.stringify(this.tokens[index])
            );
            this.errors.push(
              "Unexpected token " +
        ...
        while (
          ...
          if (
            this.tokens[index].type == "WHITESPACE" ||
            this.tokens[index].type == "COMMENT"
          ) {
            // Ignore these again.
              ...
              if (prop.type == "PROPERTY") {
                switch (prop.words[0]) {
                  ...
                  case "axis":
                    ...
                    nextBone.axis_order =
                      prop.words[prop.words.length - 1].split("");
                    break;
                  case "dof":
                    nextBone.dof = prop.words
                      .slice(1)
                      .map((s) => s.toUpperCase());
                    break;
                  case "limits":
                    // Split each constraint argument into two, then parse.
                    nextBone.limits = prop.words[1].map((l) =>
                      l.split(/,?\s+/).map(parseFloat)
                    );
                    break;
                  default:
                    console.error(
                      "[3] warning! in bonedata loop, cannot process token " +
                        JSON.stringify(this.tokens[index])
                    );
                    this.errors.push(
                      "Unexpected property token " +
                        JSON.stringify(this.tokens[index]) +
                        " on index " +
                        index +
                        " in bone data read."
                ...
                console.error(
                  "[2] warning! in bonedata loop, cannot process token " +
                    JSON.stringify(this.tokens[index])
                );
                this.errors.push(
                  "Unexpected non-property token " +
                    JSON.stringify(this.tokens[index]) +
                    " on index " +
                    index +
                    " in bone data read."
            ...
            console.error(
              "[1] warning! in bonedata loop, cannot process token " +
                JSON.stringify(this.tokens[index])
            );
            this.errors.push(
              "Unexpected token " +
        ...
        console.error(
          "warning! cannot process token " + JSON.stringify(this.tokens[index])
        );
        this.errors.push(
    ...
    function traverse(tree, parent, newPos, lastRotation, g) {
      ...
      for (const child in tree) {
        ...
        traverse(
          tree[child],
          bone,
          new THREE.Vector3(0, 0, g.bones[child].length),
          rotation,
          g
        );
      }
    ...
    for (const boneName in keydata) {
      ...
      if (boneName == this.root.name) {
        ...
        if (this.root.order.length != keydata[boneName].length) {
          ...
          this.errors.push(
            `Incorrect number of motion parameters for root node, ${keydata[boneName].length} provided.`
          );
          console.error(
            `incorrect number of motion parameters for root node, ${keydata[boneName].length} provided.`
          );
      ...
      const order =
        boneName == this.root.name ? this.root.order : this.bones[boneName].dof;
      const axis_order =
        boneName == this.root.name
          ? this.root.axis.join("")
          : this.bones[boneName].axis_order.join("");
      const axis_values =
        boneName == this.root.name
          ? this.root.orientation
          : this.bones[boneName].axis;

      let axisEuler = new THREE.Euler(
        toRadians(axis_values[0]),
        toRadians(axis_values[1]),
        toRadians(axis_values[2]),
        axis_order
      );
      ...
      const position = new THREE.Vector3().copy(
        this.three.bones[boneName].position
      );
      ...
      if (updateBones) {
        ...
        if (
          this.mmd.loaded &&
          Object.keys(this.mmd.bonemap).includes(boneName) &&
          this.mmd.bonemap[boneName] != null
        ) {
          // Attempt to puppet the MMD model as well
          const mmdbone = this.mmd.skeleton.getBoneByName(
            this.mmd.bonemap[boneName]
          );
          if (mmdbone) {
            if (boneName == this.root.name) {
              ...
              this.mmd.transforms[boneName] &&
              this.mmd.transforms[boneName].position
            ) {
            ...
            if (
              this.mmd.transforms[boneName] &&
              this.mmd.transforms[boneName].rotation
            ) {
  ...
  bake(amcFrames, amcName) {
    ...
    this.baked.clip = new THREE.AnimationClip(
      amcName,
      TIME_PER_FRAME * amcFrames.length,
      this.baked.tracks
    );

    this.baked.name = amcName;
    this.baked.mixer = new THREE.AnimationMixer(this.three.helper);
    this.baked.action = this.baked.mixer
      .clipAction(this.baked.clip)
  ...
  setFromEuler(quat, euler) {
    ...
    switch (order) {
      ...
      default:
        console.warn(
          "Inner THREE.Quaternion: .setFromEuler() encountered an unknown order: " +
            order
        );
    }
